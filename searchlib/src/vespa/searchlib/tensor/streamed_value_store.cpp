...
StreamedValueStore::get_tensor_data(EntryRef ref) const
{
    ...
    if (retval.num_subspaces * _tensor_type.dense_subspace_size() == num_cells) {
        retval.valid = true;
        return retval;
    }
    LOG(warning, "inconsistent stored tensor data: "
        "num_subspaces[%zu] * dense_subspace_size[%zu] = %zu != num_cells[%u]",
        retval.num_subspaces, _tensor_type.dense_subspace_size(),
        retval.num_subspaces * _tensor_type.dense_subspace_size(),
        num_cells);
    return retval;
}
                              ...
                              vespalib::nbostream &target) const
{
    ...
    uint32_t num_mapped_dims = _tensor_type.count_mapped_dimensions();
